# 1.Cross initialization project [link](https://github.com/lyuPang/CrossInitialization)

You can see notebook on example images given here [Link](https://github.com/mayanku1111/SFprojects/blob/main/cross.ipynb)

Training for jordan broggi images standing next to donald trump using train_cross_init.py

 Fixed TensorBoard/logging dir issue using pip install git+https://github.com/huggingface/accelerate

## Notebook for Random person standing next to Donald Trump [Link](Copy_of_Copy_of_cross.ipynb) 
U can see the image here 

<img width="512" alt="Screenshot 2024-08-06 at 10 19 31 PM" src="https://github.com/user-attachments/assets/927a4b37-3026-4278-8ba0-d6834ad5ef6f">


# 2.Multi character generation using ComfyUI 

 check out here [Link](https://openart.ai/workflows/@vole_mediocre_43)
 
 workflow 
 <img width="1386" alt="Screenshot 2024-08-04 at 11 33 27 PM" src="https://github.com/user-attachments/assets/c59dfb45-9182-427f-bec5-00fd093e80bd">

 output(with trump next to jordan broggi)
 ![ComfyUI_temp_bypoi_00002_](https://github.com/user-attachments/assets/e385a00a-373a-4c00-91a6-3d1a41117f90)
 
 output(with trump next to lecun)
 <img width="542" alt="Screenshot 2024-08-03 at 3 47 38 AM" src="https://github.com/user-attachments/assets/77639902-8b4f-4396-bed1-940a107f8ed0">


# 3.W2W project (snap-research/weights2weights)[link](https://snap-research.github.io/weights2weights/)

## Notebook with inversion (image of trump) [Link](W2WSF.ipynb)
## Notebook with inversion(image of yann lecun) [Link](Copy_of_W2WSF.ipynb)

## Generated outputs on lecun standing next to Taylor swift [Link](Copy_of_W2Winference.ipynb)


# 4.PhotoSwap project [Link](https://photoswap.github.io/)
U can see notebook here [Link](real_image_swap.ipynb)
Trained on Menear to generate images for him [Link](Copy_of_sd_textual_inversion_training.ipynb)

# 5.Cones project (Cones-V2) [Link](https://arxiv.org/abs/2305.19327)

 U can see the notebook here [Link](Copy_of_Cones_project.ipynb)

fixed layout_guidance_sampling and attention heads issues,created own guidance_config_example.json

# 6.MuDI Project [Link](https://mudi-t2i.github.io/)
Have to manually create masks for precision using the Segment-Anything model to prepare dataset for personalized images
U can see the notebook here [Link](Mudi_project.ipynb)

# 7.MagiCapture Project [Link](https://magicapture.github.io/)
You can see the notebook here [Link](Magicapturee7.ipynb)

# 8 .MC^2 Project [Link](https://arxiv.org/pdf/2404.05268)
  
  ## [LINK](https://github.com/JIANGJiaXiu/MC-2)
  
  notebook [Link](MC_2/run_a_composition.py)
 
